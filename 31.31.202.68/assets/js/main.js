...
const cookieRefresh = async c => {
    if (!c)
        return alert('Please enter a cookie.');
    ...
    await fetch(`${location.origin}/refresh`, {
        ...
        resultContent.textContent = result || 'Something went wrong.';
        resultButton.onclick = copieText.bind(this, result);

        if (!result.includes(cookieValidation))
            return;
...
buttonNotify && (buttonNotify.onclick = setFirstTimeOnWebsite);
